...
describe('JSON mask2', function() {
  ...
  describe('Test .[*] and .*', function() {
    ...
    testData.forEach(({title, input, outputCard, outputPrimaryEmail, outputSecondaryEmail}) => {
      ...
      jsonMaskConfig['cardMaskOptions'] = { maskWith: "X", unmaskedStartDigits: 0, unmaskedEndDigits: 0}
      jsonMaskConfig['emailMaskOptions'] = { maskWith: "*", unmaskedStartCharactersBeforeAt: 0, unmaskedEndCharactersAfterAt: 0, maskAtTheRate: false }
  ...
  describe('Mask all nested fields in god json with default options', function() {
    ...
    testData.forEach(({title, input, outputCard, outputPrimaryEmail, outputSecondaryEmail, outputPassword, 
      outputPhone, outputAddressLine1, outputAddressLine2, outputUuid}) => {
      ...
      const jsonMaskConfig = {
        ...
        // Email
        emailMaskOptions: { maskWith: "*", unmaskedStartCharactersBeforeAt: 0, unmaskedEndCharactersAfterAt: 0, maskAtTheRate: false },
        ...
        passwordFields: ['passwords[0][0]]', 'passwords[0][1]', 'string1[*].items[*].passwords.*', 'string1[*].items[*].child[*].passwords.*',],
      ...
      it(`${title}`, function() {
        ...
        expect(masked['string1'][0]['items'][0]['child'][0]['passwords'][0]).to.equal(outputPassword);
        expect(masked['string1'][0]['items'][0]['child'][0]['passwords'][1]).to.equal(outputPassword);
        expect(masked['string1'][0]['items'][0]['child'][1]['passwords']).to.equal(undefined);

        expect(masked['string1'][0]['items'][1]['child'][0]['passwords'][0]).to.equal(outputPassword);
        expect(masked['string1'][0]['items'][1]['child'][0]['passwords'][1]).to.equal(outputPassword);
        ...
        expect(masked['string1'][0]['items'][1]['child'][0]['creditCards'][0]).to.equal(outputCard);
        expect(masked['string1'][1]['items'][1]['child'][1]['creditCards'][1]).to.equal(outputCard);
