...
class MaskHelper {
  ...
  static getValidatedJsonMask2Configs(configs) {
    if(!configs) return Constants.defaultjsonMask2Configs;

    if(!configs.cardMaskOptions) {
      configs.cardMaskOptions = Constants.defaultCardMaskOptions;
    } else {
      this.validateCardMaskOptions(configs.cardMaskOptions);
    }

    if(!configs.emailMaskOptions) {
      configs.emailMaskOptions = Constants.defaultEmailMask2Options;
    } else {
      this.validateEmailOptions2(configs.emailMaskOptions);
    }

    if(!configs.passwordMaskOptions) {
      configs.passwordMaskOptions = Constants.defaultPasswordMaskOptions;
    } else {
      this.validatePasswordOptions(configs.passwordMaskOptions);
    }

    if(!configs.phoneMaskOptions) {
      configs.phoneMaskOptions = Constants.defaultPhoneMaskOptions;
    } else {
      this.validatePhoneOptions(configs.phoneMaskOptions);
    }

    if(!configs.stringMaskOptions) {
      configs.stringMaskOptions = Constants.defaultStringMaskOptions;
    } else {
      this.validateStringOptions(configs.stringMaskOptions);
    }
