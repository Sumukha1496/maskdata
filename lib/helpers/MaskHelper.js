...
class MaskHelper {
  ...
  static validateJSONOptions(options) {
    ...
    if(!options.maxMaskedCharactersStr || typeof(options.maxMaskedCharactersStr) !== 'number' || options.maxMaskedCharactersStr <= 0) {
  ...
  static getValidatedJsonMask2Configs(configs) {
    ...
    if(!configs.passwordMaskOptions) {
      configs.passwordMaskOptions = JSON.parse(JSON.stringify(Constants.defaultPasswordMaskOptions));
    }
