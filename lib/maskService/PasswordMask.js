...
class PasswordMask {
  static maskPassword(password, options) {
    ...
    if (options.maskedOutputLength !== undefined && options.maskedOutputLength >= 0) {
      maskPasswordLength = options.maskedOutputLength;
      if (password.length < options.maskedOutputLength) {
        password =
          password + `${options.maskWith}`.repeat(options.maskedOutputLength - password.length);
      }
    }
    ...
    const maskingCharacters =
      maskPasswordLength - options.unmaskedStartCharacters - options.unmaskedEndCharacters;
