...
class PasswordMask {
  static maskPassword(password, options) {
    ...
    if (options.fixedOutputLength !== undefined && options.fixedOutputLength >= 0) {
      maskPasswordLength = options.fixedOutputLength;
      if (password.length < options.fixedOutputLength) {
        password =
          password + `${options.maskWith}`.repeat(options.fixedOutputLength - password.length);
      }
    }
