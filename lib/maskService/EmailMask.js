...
class EmailMask {
  static maskEmail2(email, options) {
    ...
    let maskedEmail = '';
    options = options
      ? MaskHelper.mapWithDefaultValues(options, Constants.defaultEmailMask2Options)
    ...
    maskedEmail +=
      options.unmaskedStartCharactersBeforeAt >= beforeLength
        ? before
        : before.substring(0, options.unmaskedStartCharactersBeforeAt) +
    ...
    maskedEmail +=
      options.unmaskedEndCharactersAfterAt >= afterLength
        ? after
        : `${options.maskWith}`.repeat(afterLength - options.unmaskedEndCharactersAfterAt) +
          after.substring(afterLength - options.unmaskedEndCharactersAfterAt);
    return maskedEmail;
  }
}
