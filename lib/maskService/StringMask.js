...
class StringMask {
  ...
  static maskStringV2(string, options) {
    ...
    if (options.maskedOutputLength !== undefined && options.maskedOutputLength >= 0) {
      maskStringLength = options.maskedOutputLength;
      if (string.length < options.maskedOutputLength) {
        string = string + `${options.maskWith}`.repeat(options.maskedOutputLength - string.length);
      }
    }
