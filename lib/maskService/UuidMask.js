...
class UuidMask {
  static maskUuid(uuid, options) {
    if (!uuid || typeof uuid !== 'string' || !uuid.match(uuidFormat) || uuid.length !== 36)
      return uuid;
    if (!options) {
    ...
    const maskLength =
      toBeProcessedUuid.length - options.unmaskedStartCharacters - options.unmaskedEndCharacters;
    const tempMaskedUuid = start + options.maskWith.repeat(maskLength) + end;

    let maskedUuid =
      tempMaskedUuid.slice(0, 8) +
      '-' +
