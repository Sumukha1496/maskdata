...
class JsonMask {
    ...
    static async maskJSON2(obj, config) {
      const validatedConfig = MaskHelper.getValidatedJsonMask2Configs(config);
      let maskedObj = {};
      try {
        maskedObj = JSON.parse(JSON.stringify(obj));
      ...
      const maskFunctions = {
        card: CardMask.maskCard,
        email: EmailMask.maskEmail2,
        password: PasswordMask.maskPassword,
        phone: PhoneMask.maskPhone,
        string: StringMask.maskString,
        uuid: UuidMask.maskUuid,
      };
    
      const maskPromises = [];
    
      for (const fieldType in maskFunctions) {
        const fields = validatedConfig[`${fieldType}Fields`];
        const maskFunc = maskFunctions[fieldType];
        const maskOpts = validatedConfig[`${fieldType}MaskOptions`];
        for (const field of fields) {
          maskPromises.push(
            new Promise((resolve) => {
              const masked = maskFunc(JsonGetSet.get(maskedObj, field), maskOpts);
              JsonGetSet.set(maskedObj, field, masked);
              resolve();
            })
          );
