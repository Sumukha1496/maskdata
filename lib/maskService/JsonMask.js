...
class JsonMask {
   ...
   static getNestedKeys(obj, prefix = '', visited = new Set()) {
    let keys = [];
    ...
    for (let key in obj) {
      if (obj.hasOwnProperty(key)) {
        ...
        if (typeof obj[key] === 'object' && !Array.isArray(obj[key])) {
          keys = keys.concat(this.getNestedKeys(obj[key], fullPath, visited));
        } else if (Array.isArray(obj[key])) {
          for (let i = 0; i < obj[key].length; i++) {
            keys = keys.concat(this.getNestedKeys(obj[key][i], `${fullPath}[${i}]`, visited));
          }
        } else {
          keys.push(fullPath);
        }
