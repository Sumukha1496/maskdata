...
class JsonMask {
    ...
    for (const key of Object.keys(typeToFunctionMap)) {
      ...
      for (const field of validatedConfig[typeToFunctionMap[key][1]]) {
        if(field.startsWith("*") && !field.includes(".")) {
          this.maskRecursive(maskedObj, field.substring(1), key, validatedConfig[typeToFunctionMap[key][2]]);
        }
      ...
      for (let genericString of validatedConfig['genericStrings']) {
        if (genericString['fields'] && genericString['fields'].length > 0) {
          ...
          for (let field of genericString['fields']) {
            if(field.startsWith("*") && !field.includes(".")) {
              this.maskRecursive(maskedObj, field.substring(1), 'generic', validatedMaskConfig);
            }
  ...
  static maskRecursive(json, field, type, maskConfig) {
    // console.log(json, field, type, maskConfig)
    for (let key in json) {
      if(json[key] === null || json[key] === undefined) {
        continue;
      }
      if (key === field && typeof json[key] !== 'object') {
        ...
        if (value !== null && value !== undefined) {
          ...
          this.maskRecursive(json[key], field, type, maskConfig);
      }
    }
    return json;
  }
