...
class JsonMask {
          ...
          for (const arrayElement of arrayValue) {
            ...
            if (value === undefined || value === null) {
              ...
              if (
                ...
                options.maxMaskedCharactersStr < value.length
              ) {
                JsonGetSet.set(
                  arrayElement,
                  subField,
                  `${options.maskWith}`.repeat(options.maxMaskedCharactersStr)
                );
              } else {
                JsonGetSet.set(
                  arrayElement,
                  subField,
                  `${options.maskWith}`.repeat(value.toString().length)
                );
              }
            }
          }
  ...
  static mask(json, field, type, maskConfig) {
    if (field.includes('.*') && !field.endsWith('.*')) {
      console.warn('Ignoring the invalid field: ', field);
      return;
    }
